...
const Admin = () => {
  ...
  const updateStatus = async (complaintId, status) => {
    try {
      const { data } = await API.put(`/complaints/${complaintId}/status`, { status });
      if (data.success) {
  ...
  const handleDeleteAgent = async (agentId, agentName) => {
    if (!window.confirm(`Are you sure you want to remove agent "${agentName.toUpperCase()}"? Any active tickets assigned to this agent will be unassigned.`)) return;

    try {
      const { data } = await API.delete(`/users/${agentId}`);
      if (data.success) {
        showToast(`Agent "${agentName.toUpperCase()}" removed successfully`, "success");
        fetchData();
      }
    } catch (err) {
  ...
  const escalatedClaims = complaints.filter((c) => c.status === "escalated").length;
  const openClaims = complaints.filter((c) => c.status === "open").length;
  const inProgressClaims = complaints.filter((c) => c.status === "in-progress").length;
  const resolvedClaims = complaints.filter((c) => c.status === "resolved" || c.status === "closed").length;
  ...
  const agentWorkloadCompiled = agents.map((agent) => {
    const agentTickets = complaints.filter((c) => c.assignedAgent && c.assignedAgent._id === agent._id);
    const active = agentTickets.filter((c) => ["open", "in-progress", "escalated"].includes(c.status)).length;
    const completed = agentTickets.filter((c) => ["resolved", "closed"].includes(c.status)).length;
    const rated = agentTickets.filter((c) => c.rating !== null);
    const ratingSum = rated.reduce((acc, curr) => acc + curr.rating, 0);
    const averageRating = rated.length > 0 ? (ratingSum / rated.length).toFixed(1) : "N/A";
  ...
  const filteredComplaints = complaints.filter((c) => {
    ...
    if (filterAgent === "unassigned") {
      matchesAgent = !c.assignedAgent;
    } else if (filterAgent) {
      matchesAgent = c.assignedAgent && c.assignedAgent._id === filterAgent;
    }
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-300 text-sm mt-1 max-w-xl">
              Monitor customer claims, manage agent staff workloads, reassign cases, and oversee CSAT ratings.
            </p>
          </div>
          <button
      ...
      {currentView === "overview" && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              { label: "Global Claims", count: totalClaims, color: "text-slate-700 bg-white" },
              { label: "Unassigned", count: unassignedClaims, color: "text-rose-600 bg-white" },
              { label: "Escalated", count: escalatedClaims, color: "text-red-600 bg-white border border-red-100" },
              { label: "In Progress", count: inProgressClaims, color: "text-amber-600 bg-white" },
              { label: "Resolved Total", count: resolvedClaims, color: "text-emerald-600 bg-white" },
            ].map((stat, idx) => (
              <div
                key={idx}
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-1 space-y-6">
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider border-b pb-3">Priority Distribution</h3>
                <div className="space-y-3.5">
                  {[
                    { label: "Urgent", count: priorityCounts.urgent, barColor: "bg-rose-500" },
                    { label: "High", count: priorityCounts.high, barColor: "bg-orange-500" },
                    { label: "Medium", count: priorityCounts.medium, barColor: "bg-yellow-500" },
                    { label: "Low", count: priorityCounts.low, barColor: "bg-blue-500" },
                  ].map((p, idx) => {
                    ...
                    return (
                      <div key={idx} className="space-y-1">
                        ...
                        <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                          <div className={`h-full ${p.barColor}`} style={{ width: `${pct}%` }} />
                        </div>
                      </div>
                    );
                  })}
              ...
              {/* Status Bar Chart */}
              <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider border-b pb-3">Status Breakdown</h3>
                <div className="space-y-3.5">
                  {[
                    { label: "Open / Unassigned", count: openClaims, barColor: "bg-rose-500" },
                    { label: "In Progress", count: inProgressClaims, barColor: "bg-amber-500" },
                    { label: "Escalated", count: escalatedClaims, barColor: "bg-red-500" },
                    { label: "Resolved / Closed", count: resolvedClaims, barColor: "bg-emerald-500" },
                  ].map((s, idx) => {
                    ...
                    return (
                      <div key={idx} className="space-y-1">
                        ...
                        <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                          <div className={`h-full ${s.barColor}`} style={{ width: `${pct}%` }} />
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="md:col-span-2 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
              <div className="flex justify-between items-center border-b pb-3">
                <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Agent Workloads Overview</h3>
                <button
              ...
              {agentWorkloadCompiled.length > 0 ? (
                <div className="divide-y divide-slate-100">
                  {agentWorkloadCompiled.map((agent) => (
                    <div key={agent._id} className="py-3 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-sm font-bold text-slate-850 tracking-wide">{agent.name.toUpperCase()}</h4>
                          <span className="text-xs text-slate-400 font-mono">{agent.email}</span>
                        </div>
                      </div>

                      <div className="flex items-center gap-6 text-right">
                        <div>
                          <span className="text-[10px] text-slate-400 block uppercase font-bold">Active Cases</span>
                          <span className="text-sm font-bold text-slate-700">{agent.activeCount} active</span>
                        </div>
                        <div>
                          <span className="text-[10px] text-slate-400 block uppercase font-bold">CS Rating</span>
                ...
                <div className="text-center py-12 flex flex-col items-center justify-center gap-2 text-slate-400">
                  ...
                  <p className="text-xs">No support agents registered. Create an account to begin distributing cases.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          {loading ? (
            <div className="bg-white p-24 rounded-2xl border border-slate-100 shadow-sm text-center flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-sm text-slate-500 font-medium">Fetching registry...</p>
            </div>
              ...
              {filteredComplaints.map((c) => (
                ...
                >
                  <div>
                    ...
                    <h3 className="font-bold text-slate-800 text-lg mt-2.5">{c.title}</h3>
                    ...
                    <div className="mt-4 text-xs space-y-1">
                      ...
                      <div className="font-bold text-slate-700">
                        {c.customer?.name ? c.customer.name.toUpperCase() : "UNKNOWN"} <span className="font-normal font-mono text-slate-500">({c.customer?.email || "No email"})</span>
                      </div>
                    </div>

                    {/* Assign section */}
                    <div className="mt-4 pt-3 border-t border-slate-50">
                      <div className="text-[10px] text-slate-400 font-bold uppercase mb-1.5">Support Specialist</div>
                      <select
                      ...
                      >
                        <option value="" disabled={!!c.assignedAgent}>
                          {c.assignedAgent ? "Reassign Specialist" : "Unassigned - Click to allocate"}
                        </option>
                        {agents.map((a) => (
                          <option key={a._id} value={a._id}>
                            {a.name.toUpperCase()} ({c.assignedAgent?._id === a._id ? "Current" : "Assign"})
                          </option>
                        ))}
                      </select>
                    </div>
            ...
            <div className="bg-white rounded-2xl border border-dashed border-slate-200 py-24 text-center flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-base font-bold text-slate-700">No registry matches found</p>
      ...
      {currentView === "manage-agents" && (
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-slate-800">Support Desk Staff</h2>
              <p className="text-xs text-slate-500 mt-0.5">Workloads, customer service performance ratings, and accounts registry.</p>
            </div>
            <button
              onClick={() => setShowAgentModal(true)}
          ...
          {agentWorkloadCompiled.length > 0 ? (
                ...
                <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                  {agentWorkloadCompiled.map((agent) => (
                    <tr key={agent._id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="py-4 px-6 font-bold text-slate-800 tracking-wide">{agent.name.toUpperCase()}</td>
                      <td className="py-4 px-6 font-mono text-xs text-slate-500">{agent.email}</td>
                      <td className="py-4 px-6 text-center font-bold text-indigo-600">{agent.activeCount}</td>
                      <td className="py-4 px-6 text-center text-slate-500">{agent.completedCount}</td>
            ...
            <div className="py-20 text-center flex flex-col items-center justify-center gap-3 text-slate-400">
              ...
              <p className="text-sm font-semibold text-slate-700">No support agents configured</p>
              <button
                onClick={() => setShowAgentModal(true)}
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 space-y-4">
                      <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                        <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Registry Metadata</h3>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-xs">
                        ...
                        <div>
                          <p className="font-bold text-slate-700 mt-1">
                            {activeComplaint.customer?.name ? activeComplaint.customer.name.toUpperCase() : "UNKNOWN"} ({activeComplaint.customer?.email})
                          </p>
                        </div>
                        <div>
                          <p className="font-semibold text-slate-400">ALLOCATED SPECIALIST</p>
                          <select
                            onChange={(e) => handleAssign(activeComplaint._id, e.target.value)}
                      ...
                      {messagesLoading ? (
                        ...
                        <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1">
                          {messages.map((m) => {
                            ...
                            const roleLabel = m.senderRole === "customer" ? "Customer" : m.senderRole === "agent" ? "Specialist" : "You (Admin)";
                            return (
                              <div
                                key={m._id}
                        ...
                        <div className="py-12 border border-dashed border-slate-100 rounded-2xl text-center flex flex-col items-center justify-center gap-2 text-slate-400">
                          ...
                          <p className="text-xs">No messages yet. Send a note below to join the discussion.</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="border-t border-slate-100 px-6 py-4 bg-slate-50 shrink-0">
                    <form onSubmit={handleSendMessage} className="flex gap-2">
                      <input
                        ...
                        placeholder={activeComplaint.status === "closed" ? "This complaint is closed" : "Message thread as Administrator..."}
                        value={newMessage}
        ...
        >
          ...
          >
            <div className="flex items-start justify-between mb-2">
              <h2 className="text-xl font-bold text-slate-800">Register Support Agent</h2>
              <button
                onClick={() => setShowAgentModal(false)}
            ...
            <p className="text-xs text-slate-500 mb-6">
              Create login credentials for a new support specialist. They will immediately appear in allocation lists.
            </p>
